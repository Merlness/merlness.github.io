...
(defn grid-after-move
  ([move {:keys [player-1 player-2 size moves]}]
   (grid-after-move move (:token player-1) (:token player-2) size moves)) ;this line is not tested
...
(defn create-player [position unavailable-token] ;multi
...
(defmulti get-move (fn [player _opponent _grid] (:kind player))) ;used in web ttt
(defmethod get-move :human [_player _opponent grid] (ui/get-move grid)) ;ready
